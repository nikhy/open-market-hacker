...
class SMA200(bt.Strategy):
    ...
    def __init__(self):
        self.sma_24 =  bt.ind.SimpleMovingAverage(self.data.close,period =24)
        self.sma_12 =  bt.ind.SimpleMovingAverage(self.data.close,period =12)
        self.sma_6 =  bt.ind.SimpleMovingAverage(self.data.close,period =6)
        
        # Keep a reference to the "close" line in the data[0] dataseries

    def next(self):      
        if  (self.bought == False and self.sma_24[0] > self.sma_6[0]
            and self.sma_12[0] > self.sma_6[0]) :
            self.bought = True
            self.buy()
...
if __name__ == '__main__':
    ...
    output.to_excel('output/sma200_2018_to_2020dec.xlsx',header = ['Symbol','P/L %'])
